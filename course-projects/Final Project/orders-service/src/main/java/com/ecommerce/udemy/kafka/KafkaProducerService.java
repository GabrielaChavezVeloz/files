...
public class KafkaProducerService {
    ...
    public void sendMessage(OrderMessage orderMessage) {
        ListenableFuture<SendResult<String, OrderMessage>> future = kafkaTemplate.send("t-order", UUID.randomUUID().toString(), orderMessage);
        future.addCallback(new ListenableFutureCallback<SendResult<String, OrderMessage>>() {

			@Override
			public void onFailure(Throwable ex) {
				log.error("Error trying to send Order Message to course service: " + ex.getMessage());
			}

			@Override
			public void onSuccess(SendResult<String, OrderMessage> result) {
				log.info("Course message sent");
			}
		});
    }
}
